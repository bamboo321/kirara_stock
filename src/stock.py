...
class stock:
    ...
    def update(self) -> None:
        ...
        self.updated_time = datetime.strptime(
            self._fetch_updated_time(), '%Y年%m月%d日 %H:%M')

    def _fetch_table(self) -> iter:
        goods_table = self.soup.find(
            'table', class_='goods_list').find('tbody')
        # HACK: find_all_nextでイテレータを作成する?
        goods_rows = goods_table.find_all('tr')

        def row_iter(goods_rows) -> iter:
            for row in goods_rows:
                # HACK: ...
                striped_row = [elm.get_text().strip()
                               for elm in row.find_all('th')]
                yield striped_row
    ...
    def _fetch_updated_time(self) -> str:
        text = self.soup.find(class_='goodslist_exp').find_all('p')[
            1].find('strong').get_text()
